...
public class Parser {
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            JStatement dothis = statement();
            mustBe(WHILE);
            JExpression whilethis = parExpression();
            mustBe(SEMI);
            return new JDoStatement(line, dothis, whilethis);
        } else if (have(FOR)){
            mustBe(LPAREN);
            JVariableDeclaration initial = localVariableDeclarationStatement();
            JExpression fors = expression();
            mustBe(SEMI);
            JStatement updates = statementExpression();
            mustBe(RPAREN);
            JStatement statements = statement();
            return new JForStatement(line, initial, fors, updates, statements);
            ...
            return new JWhileStatement(line, cond, statement);
        } else if(have(SWITCH)){
/*           JExpression parExp = parExpression();
            ArrayList<SwitchStatementGroup> switchBlockStatementGroup =
            ...
            mustBe(RCURLY);
            return new JSwitchStatement(line, parExp, switchBlockStatementGroup);
*/
            /*
...
/*
    ...
    private SwitchStatementGroup switchBlockStatementGroup() {
        ...
        return new SwitchStatementGroup(line, switchLabels, blockStatement);
    }
...
*/
     ...
     *                                    { QUESTION inclusiveOrExpression}
     * </pre>
     *
    ...
    private JExpression conditionalAndExpression() {
        ...
        while (more) {
            if (have(LAND)) {
                ...
                return new JLogicalOrOp(line, lhs, inclusiveOrExpression());
            } else {
                more = false;
            }
        }
    ...
    /**
     * Parses a conditional-exclusive-or expression and returns an AST for it.
     *
     * <pre>
    ...
    private boolean seeReferenceType() {
        if (see(IDENTIFIER)) {
            ...
            scanner.recordPosition();
            if (have(BOOLEAN) || have(CHAR) || have(DOUBLE_LITERAL) || have(INT) || have(LONG_LITERAL)) {
